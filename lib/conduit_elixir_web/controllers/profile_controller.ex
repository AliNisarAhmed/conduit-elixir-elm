defmodule ConduitElixirWeb.ProfileController do
  ...
  plug :require_authenticated_user when action in [:show, :follow_user, :unfollow_user]

  def show(%Plug.Conn{assigns: assigns} = conn, %{"username" => username}) do
    case Profiles.get_profile(username, assigns.current_user) do
      nil -> {:error, :not_found}
